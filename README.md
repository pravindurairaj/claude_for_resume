...
AI-assisted resume tailoring and LinkedIn job scraping workspace. Built for use with GitHub Copilot Chat as an agent — two skills handle the full job-hunt pipeline: scrape relevant jobs, then tailor a resume to a chosen listing.
...
|-------------|---------|
| Python | 3.10+ |
| GitHub Copilot Chat | Latest (agent mode) |
| VS Code | 1.90+ |
...
Create `.github/instructions/career-profile-{YourName}.instructions.md` with:
- **Contact Information** — name, email, phone, LinkedIn, location
- **Target Roles** — list of role titles to search and match against
- **Target Industries** — for fit scoring

See existing profiles in `.github/instructions/` as reference.
...
│
├── .github/
│   ├── instructions/                          # Career profiles (one per user)
│   │   ├── career-profile-pravin.instructions.md
│   │   └── career-profile-navya.instructions.md
│   │
│   ├── skills/
│   │   ├── job-scraper/
...
│   │       └── scripts/
│   │           ├── md-to-docx.py              # Markdown → .docx converter (Calibri Light 11pt, ATS-safe)
│   │           ├── extract-resume.py          # .docx/.pdf → Markdown extractor
│   │           ├── tailor-resume.py           # Deterministic keyword-match tailoring → .docx directly
│   │           ├── batch-pipeline.py          # Autonomous batch orchestrator (manifest → all .docx)
│   │           ├── batch-job-reader.py        # Excel → JSON manifest generator
│   │           └── log-application.py         # Append row to resumes_created.xlsx
│   │
│   ├── Users/                                 # Master resumes + per-company context
│   │   ├── Pravin/
│   │   │   ├── Pravin_Resume.md
│   │   │   └── companies/                     # Per-job: JD summary + keyword match report
│   │   └── Navya/
│   │       ├── Navya_Resume.md
│   │       └── companies/
│   │
│   ├── agents/
│   │   └── job-search.agent.md                # Full pipeline agent (scrape → manifest → batch)
│   │
│   ├── prompts/                               # Slash-command autocomplete prompts
│   │   ├── job-scraper-pravin.prompt.md
│   │   ├── job-scraper-navya.prompt.md
│   │   ├── resume-tailor-pravin.prompt.md
│   │   └── resume-tailor-navya.prompt.md
│   │
│   ├── context/                               # Developer notes / architecture docs
│   │   ├── job_scraper.md
│   │   └── resume_tailor.md
│   │
│   └── requirements.txt                       # Python dependencies
│
├── Pravin/                                    # Pravin's outputs
...
│
├── Navya/                                     # Navya's outputs (same structure)
│   ├── JobSearch/
│   ├── Resumes/
│   └── History/
│
